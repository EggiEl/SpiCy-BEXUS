...
file_path = 'data\\Temp_2W5_cooled.csv'  # Path to your CSV file
...
data['temp_ambient[°C]'] = pd.to_numeric(data['temp_ambient[°C]'].str.strip(), errors='coerce')
...
# data['temp_ambient[°C]'] = data['temp_ambient[°C]'].rolling(window=20).mean()
...
plt.plot(data['timestamp[ms]'], data['temp_ambient[°C]'], label='Temperature Out (°C)', color='red', linestyle='-', linewidth=1)
