...
static unsigned int CONNECTIONTIMEOUT = 3000; /*Conntection Timeout of the tcp client*/
...
void TCP_setup_client()
{
  ...
  debugf_yellow("<TCP_setup_client>\n");
  ...
  if (Ethernet.hardwareStatus() && Ethernet.linkStatus())
  {
    TCP_init = 1;
...
void TCP_print_info()
{
  ...
  case 2:
    ...
    break;
  default:
    debugf_red("error in code. Ethernet.linkStatus() = %i\n", cabletest_buff);
    cabletest_buff = 0;
    break;
  }
  debugf("-server:");
 ...
 */
void TCP_test_manually(int nPackets, unsigned int nTries)
{
  MESSURETIME_START
